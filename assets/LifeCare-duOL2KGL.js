import{S as ae,r as se,M as ie,_ as ne}from"./vue3-grid-layout-CuLf5Vgn.js";import{d as re,r as h,i as oe,j as ce,k as ue,c as M,a as n,w as c,u as i,g as L,n as E,l as S,m as de,v as pe,p as P,F as B,h as C,q as me,s as j,f as w,t as D,T as fe,x as k,I,S as Y,y as F,z as ge,B as T,D as he,A as G,E as ve,G as ke}from"./index-BQmXcYFr.js";const _e=`<!DOCTYPE html>
...
`).map(o=>o.trim()):[]),X=()=>{x.value=!x.value},K=()=>{O.value.validate().then(async()=>{q.value++,H.value="",z.value=!0,Q()}).catch(o=>{C.error(o)})},Q=async()=>{const{base_url:o,count_image:t,size_screen:r,isCustomListImageLink:l}=e,d=(l?$.value:Array.from({length:t}).map((s,a)=>`${o}${a<9?"0"+(a+1):a+1}.png`)).map((s,a)=>new Promise((b,g)=>{const p=new Image;p.src=s,p.onload=function(){b({index:a+1,height:this.height,width:this.width,src:s})},p.onerror=function(){g("Error when load image")}})),_=await Promise.all(d).catch(s=>{C.error(s)}),v=_==null?void 0:_.reduce((s,a)=>{const{width:b}=a;if(b===r)s.push(a);else{let g=s[s.length-1];Array.isArray(g)?g.reduce((p,V)=>p+V.width,0)===r?s=[...s,[a]]:(g=s.pop(),s=[...s,[...g,a]]):s=[...s,[a]]}return s},[]);e.formDataTemplate=v;let f=0;const y=v==null?void 0:v.reduce((s,a)=>{if(Array.isArray(a)){let b=[],g=0;a.forEach((p,V)=>{b.push({x:g,y:f,w:p.width,h:p.height,i:`image_${p.index}`,label:`${p.index}`,src_image:p.src}),g+=p.width}),s.push(...b),f+=a[0].height}else s.push({x:0,y:s[s.length-1]?f:0,w:r,h:a.height,i:`image_${a.index}`,label:`${a.index}`,src_image:a.src}),f+=a.height;return s},[]);A.value=y};function Z(){if(!A.value.length){C.error("Please submit data first!");return}R.value=setTimeout(()=>{const o=e.formDataTemplate.map((r,l)=>Array.isArray(r)?r.map((u,d)=>({...u,...e.data.find(_=>_.src===u.src)})):{...r,...e.data.find(u=>u.src===r.src)});let t="";o.forEach((r,l)=>{if(Array.isArray(r)){const u=r.map((d,_)=>{if(d.isFullImage)return d.full.link?m.fullHaveLink.replace("{{src}}",d.src).replace("{{link}}",d.full.link).replace("{{target_blank}}",d.full.isTargetBlank?'target="_blank"':'target="_self"').replace("{{spanBlind}}",d.full.spanBlind||""):m.fullNotLink.replace("{{src}}",d.src);{const v=d.absolute.map((f,y)=>{var a;let s=(a=f.label_position)==null?void 0:a.replace(/\n/g,"");return m.onlyLink.replace("{{link}}",f.link).replace("{{position}}",s||"position: absolute;").replace("{{target_blank}}",f.isTargetBlank?'target="_blank"':'target="_self"').replace("{{spanBlind}}",f.spanBlind||"")});return m.absoluteLinks.replace("{{src}}",d.src).replace("{{list_link}}",v.join(""))}});t+=m.grid.replace("{{col}}",r.length+"").replace("{{list_link}}",u.join(""))}else{const{src:u,width:d,height:_,isFullImage:v,absolute:f,full:y}=r;if(v)y.link?t+=m.fullHaveLink.replace("{{src}}",u).replace("{{link}}",y.link).replace("{{target_blank}}",y.isTargetBlank?'target="_blank"':'target="_self"').replace("{{spanBlind}}",y.spanBlind||""):t+=m.fullNotLink.replace("{{src}}",u);else{const s=f.map((a,b)=>{var p;let g=(p=a.label_position)==null?void 0:p.replace(/\n/g,"");return m.onlyLink.replace("{{link}}",a.link).replace("{{position}}",g||"position: absolute;").replace("{{target_blank}}",a.isTargetBlank?'target="_blank"':'target="_self"').replace("{{spanBlind}}",a.spanBlind||"")});t+=m.absoluteLinks.replace("{{src}}",u).replace("{{list_link}}",s.join(""))}}}),t=m.root.replace("{{template}}",t).replace("{{style}}",m.style[e.size_screen===1080?"mo":"pc"]),N({template:t,nameFile:`${e.size_screen===1080?"mo":"pc"}_bos_${e.base_url.split("/").reverse()[1]}.html`}),W.value=!1},1e3)}function ee({index:o,data:t}){console.log("CHANGE DATA",o),e.data[o]=t}const N=({template:o,nameFile:t})=>{const r=new Blob([o],{type:"text/html"});me.saveAs(r,t||`${e.size_screen===1080?"mo":"pc"}_bos_${e.base_url.split("/").reverse()[1]}.html`)},te=()=>{const o=e.size_screen===1080?ye:_e,t=e.size_screen===1080?"MO":"PC",r=e.base_url.split("/").reverse()[1];O.value.validate().then(async()=>{N({template:o.replace("{{bos_file_path}}",`/uxtech/linkpage/${j().startOf("day").format("YYYYMM")}/include/${t.toLowerCase()}_bos_${r}.html`).replace("{{title}}",e.title||""),nameFile:`${t.toLowerCase()}_${r}_${j().startOf("day").format("MMDD")}.html`})}).catch(l=>{C.error(l)})},le=()=>{let o=` ,{
    "UA": "EVENT",
    "1Depth": "{{title}}",
    "2Depth": "{{device}}",
    "3Depth": "",
